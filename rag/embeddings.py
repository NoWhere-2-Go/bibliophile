from typing import List, Optional
import numpy as np
import requests
import os
import time
import logging

logger = logging.getLogger(__name__)


class EmbeddingModel:
    """Embedding client that calls a local Ollama HTTP endpoint.

    Always uses the `nomic-embed-text` model by default. Ollama must be
    running locally (default http://localhost:11434) and host the model.
    
    Can also support other embedding providers if API-compatible.
    """

    def __init__(
        self,
        model_name: str = "nomic-embed-text",
        base_url: Optional[str] = None,
        max_retries: int = 3,
        timeout: float = 30.0
    ):
        self.model = model_name
        self.base_url = base_url or os.environ.get("OLLAMA_BASE_URL", "http://localhost:11434")
        self.max_retries = max_retries
        self.timeout = timeout
        logger.info(f"Initialized EmbeddingModel: {model_name} at {self.base_url}")

    def embed(self, texts: List[str], batch_size: Optional[int] = None) -> np.ndarray:
        """Embed a list of texts. Handles retries and different API response formats.
        
        Args:
            texts: List of strings to embed.
            batch_size: Optional batch size (not used here, for API consistency).
        
        Returns:
            Numpy array of embeddings, shape (len(texts), embedding_dim).
        """
        if not texts:
            raise ValueError("No texts provided for embedding")
        
        payload = {"model": self.model, "input": texts}
        
        # Try multiple endpoint patterns and retry on failure
        endpoints = [
            f"{self.base_url}/api/embed",
            f"{self.base_url}/embed",
        ]
        
        for attempt in range(self.max_retries):
            for endpoint in endpoints:
                try:
                    logger.debug(f"Embedding {len(texts)} texts (attempt {attempt+1}/{self.max_retries})")
                    resp = requests.post(
                        endpoint,
                        json=payload,
                        timeout=self.timeout
                    )
                    resp.raise_for_status()
                    data = resp.json()
                    
                    # Handle various response formats
                    embeddings = self._extract_embeddings(data)
                    if embeddings is not None:
                        return embeddings
                    
                except requests.exceptions.Timeout:
                    logger.warning(f"Timeout at {endpoint} (attempt {attempt+1})")
                    time.sleep(2 ** attempt)  # Exponential backoff
                    continue
                except requests.exceptions.ConnectionError:
                    logger.warning(f"Connection error at {endpoint} (attempt {attempt+1})")
                    time.sleep(2 ** attempt)
                    continue
                except Exception as e:
                    logger.debug(f"Error at {endpoint}: {e}")
                    continue
        
        raise RuntimeError(
            f"Failed to get embeddings after {self.max_retries} attempts. "
            f"Ensure Ollama is running with model '{self.model}'. "
            f"Test with: curl -X POST 'http://localhost:11434/api/embed' "
            f"-H 'Content-Type: application/json' "
            f"-d '{{\"model\":\"{self.model}\",\"input\":[\"test\"]}}'"
        )

    def _extract_embeddings(self, data: dict) -> Optional[np.ndarray]:
        """Extract embeddings from various API response formats."""
        try:
            if isinstance(data, dict):
                # Ollama format: {"embeddings": [[...], ...]}
                if "embeddings" in data:
                    embs = data["embeddings"]
                    if embs and isinstance(embs[0], list):
                        return np.array(embs, dtype="float32")
                
                # OpenAI format: {"data": [{"embedding": [...]}, ...]}
                if "data" in data and isinstance(data["data"], list):
                    first = data["data"][0]
                    if isinstance(first, dict) and "embedding" in first:
                        embs = [d["embedding"] for d in data["data"]]
                        return np.array(embs, dtype="float32")
                    if isinstance(first, list):
                        return np.array(data["data"], dtype="float32")
            
            # Raw list format
            if isinstance(data, list) and data and isinstance(data[0], list):
                return np.array(data, dtype="float32")
        except Exception as e:
            logger.debug(f"Error extracting embeddings: {e}")
        
        return None

    def health_check(self) -> bool:
        """Check if the embedding service is available."""
        try:
            resp = requests.post(
                f"{self.base_url}/api/embed",
                json={"model": self.model, "input": ["test"]},
                timeout=5.0
            )
            resp.raise_for_status()
            logger.info(f"✓ Embedding service healthy at {self.base_url}")
            return True
        except Exception as e:
            logger.error(f"✗ Embedding service unreachable: {e}")
            return False


def batch_embed(
    model: EmbeddingModel,
    texts: List[str],
    batch_size: int = 64
) -> np.ndarray:
    """Embed texts in batches to manage memory.
    
    Args:
        model: EmbeddingModel instance.
        texts: List of texts to embed.
        batch_size: Number of texts per batch.
    
    Returns:
        Numpy array of all embeddings.
    """
    if not texts:
        raise ValueError("No texts to embed")
    
    out = []
    total = len(texts)
    
    for i in range(0, total, batch_size):
        batch = texts[i : i + batch_size]
        start_idx = i
        end_idx = min(i + batch_size, total)
        
        logger.info(f"Embedding batch {start_idx}-{end_idx}/{total}")
        batch_embeddings = model.embed(batch)
        out.append(batch_embeddings)
    
    result = np.vstack(out)
    logger.info(f"Successfully embedded {total} texts: shape {result.shape}")
    return result

